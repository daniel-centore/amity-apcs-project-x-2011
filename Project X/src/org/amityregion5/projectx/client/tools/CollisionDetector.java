...
import org.amityregion5.projectx.client.entities.Entity;
...
public class CollisionDetector {
    // what level of opacity (0 = transparent, 255 = completely opaque) is
    // considered opaque for pixel collisions
    public static final int OPACITY_THRESHOLD = 100;

    /**
     * Checks if two entities would collide if entity a were to move First
     * checks if their surrounding boxes collide, then uses the pixel method if
     * so
     * 
     * @param a Entity a
    ...
    {
        return (hasRectangleCollision(a, xOffset, yOffset, b) && hasOutlineCollision(a, xOffset, yOffset, b));
    }

    /**
     * Checks if two entity's bounding boxes would collide if entity a were to
     * move (fast)
     * 
     * @param a Entity a
     * @param xOffset How much it moves on the x and y axis
    ...
    /**
     * Checks if two entity's would collide by pixel if entity a were to move
     * (slow)
     * 
     * @param a Entity a
     * @param xOffset How much it moves on the x and y axis
